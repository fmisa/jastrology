...
public class ChineseCalendarGB {
   ...
   public String getDateString() {
      ...
      if (gregorianDate==sectionalTerm) {
         str = " "+sectionalTermNames[gregorianMonth-1];//original is gregorianMonth-1, i'm afraid should be chineseMonth
   ...
   public String getChineseDateString() {
      StringBuffer str =new StringBuffer( stemNames[(chineseYear-1)%10]
         + branchNames[(chineseYear-1)%12]+"年");
      if ( chineseMonth>0) {
      	 str.append(chineseMonthNames[chineseMonth-1]+"月");
      }else if(chineseMonth<0) {
      	 str.append("润"+chineseMonthNames[-chineseMonth-1]+"月");
      }
      str.append(chineseDateNames[chineseDate-1]);
      if (gregorianDate==sectionalTerm) {
         str.append("("+sectionalTermNames[gregorianMonth-1]+")");
      } else if (gregorianDate==principleTerm) {
         str.append("("+principleTermNames[gregorianMonth-1]+")");
      }
      return str.toString();
   }
   ...
   public static ChineseCalendarGB computeChineseToGregorian(int year,int month,int day){
       ...
       c.computeSolarTerms();
       int count=0;
       while(count<100 && (cm!=c.chineseMonth || cd!=c.chineseDate)){
                c.rollUpOneDay();
           count++;
       }
       return c;
