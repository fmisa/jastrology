...
/*
   ...
   Thomas Mack, mack@idb.cs.tu-bs.de, 23rd of April 2001
...
//#else
  static final String PLSEL_A="0123456789mtABCDEFGHIJKLMNOPQRSTUVWXYZw";
//#endif /* ASTROLOGY */
...
//      sid_mode = atol(argv[i]+4);
        ...
        try {
          ...
          hous_long=Double.valueOf(hf).doubleValue();
          hLonIsSet=true;
          h=h.substring(idx+1);
          idx=h.indexOf(',');
          hf=h.substring(0,idx);
          hous_lat=Double.valueOf(hf).doubleValue();
          hLatIsSet=true;
          sout=h.substring(idx+1);
        } catch (NumberFormatException nfe) {
          if (!hLonIsSet) { hous_long=0; } else { hous_lat=0; }
...
//#else
    if ((smod & (SMOD_OCCULT | SMOD_RISE)) != 0) {
//#endif /* ASTROLOGY */
...
//            sout="\nNutation        "+s1+gap+s2;
          ...
          if (do_houses) {
            if (!universal_time) {
              ...
              s1=dms(hous_long, round_flag);
              s2=dms(hous_lat, round_flag);
...
//#endif /* NO_RISE_TRANS */
        ...
        if (do_houses) {
          ...
          if (Character.toLowerCase((char)ihsy) == 'g')
            nhouses = 36;
          iofs = nhouses + 1;
          iflgret = sw.swe_houses(t,iflag, hous_lat, hous_long, ihsy, cusp, cusp, iofs);
          if (iflgret < 0) {
            if (!serr.toString().equals(serr_save.toString())) {
              sout="error: "+serr.toString()+"\n";
              System.out.print(sout);
...
//                  sout=spnam.substring(0,Math.min(3,spnam.length()))+"-"+
//                       spnam2.substring(0,Math.min(3,spnam2.length()));
...
//                  sout=(spnam+"               ").substring(0,Math.max(spnam.length(),15));
          }
          System.out.print(sout);
          break;
      case (int)'J':
...
//#else
    if ((smod & (SMOD_LOCAL|SMOD_RISE))!=0)
//#endif /* ASTROLOGY */
...
//#ifndef NO_RISE_TRANS
      if ((smod & SMOD_RISE)!=0) {
        dtmp.val=tret[0];
        sstar=new StringBuffer(star);
        if (sw.swe_rise_trans(t_ut, ipl, sstar, whicheph, SweConst.SE_CALC_RISE, geopos, 1013.25, 10, dtmp, serr) != SweConst.OK) {
          do_printf(serr);
          System.exit(0);
        } 
        star=sstar.toString();
        ...
        dtmp.val=tret[1];
        sstar=new StringBuffer(star);
        if (sw.swe_rise_trans(t_ut, ipl, sstar, whicheph, SweConst.SE_CALC_SET, geopos, 1013.25, 10, dtmp, serr) != SweConst.OK) {
          do_printf(serr);
          System.exit(0);
        } 
        star=sstar.toString();
