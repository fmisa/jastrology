...
//#ifdef EXTPRECISION
    double extPrecision = tc.getPrecisionFactor();
//#endif /* EXTPRECISION */
...
//System.out.println(degPrec + " " + timePrec);
//#ifdef EXTPRECISION
    timePrec /= extPrecision;
//#endif /* EXTPRECISION */
...
//#ifdef TEST_ITERATIONS
      iterateCount++;
//#endif /* TEST_ITERATIONS */
      while (tc.rollover && val >= tc.rolloverVal) { val -= tc.rolloverVal; }
...
//              (Math.abs(offset-val)<degPrec) || // creates problems when trying to distinguish one transit from another transit...
      ...
      if (found) { // Return an interpolated value, but not prior to (after)
        ...
        if (Math.abs(val - lastVal) > 300. && tc.rollover) {   // How to do it formally correct???
