...
//#ifdef TRACE0
    Trace.level++;
    Trace.trace(Trace.level, "FileData.clearData()");
//#endif /* TRACE0 */
    int j;
    fnam="";
    fversion=0;
...
//#ifdef TRACE0
    Trace.level++;
    Trace.trace(Trace.level, "FileData.read_const(int, StringBuffer, SwissData)");
//#ifdef TRACE1
...
//#endif /* TRACE0 */
       ...
       **********************************************************/
      fdp.sweph_denum=read4(fptr, fpos, false, freord, fendian);
      /*************************************
...
//#ifdef TRACE0
    Trace.level++;
    Trace.trace(Trace.level, "FileData.swi_crc32(byte[], int)");
//#ifdef TRACE1
...
//#ifdef TRACE0
    Trace.level++;
    Trace.trace(Trace.level, "FileData.init_crc32()");
//#endif /* TRACE0 */
    long i, j;
    long c;
...
//#ifdef TRACE0
    Trace.level++;
    Trace.trace(Trace.level, "FileData.get_new_segment(SwissData, double, int, int, StringBuffer)");
//#ifdef TRACE1
...
//#endif /* TRACE0 */
   ...
   */
//#if 0
private byte[] do_fread(void *trg, int size, int count, int corrsize, FILE *fp, int32 fpos, int freord, int fendian, int ifno, char *serr)
throws IOException
{
  int i, j, k;
  int totsize;
  ...
  try {
    ...
    if (freord == 0 && size == corrsize) {
      fread((void *) targ, (size_t) totsize, 1, fp);
      fp.read(,,,);
      return ...;
    } else {
      fread((void *) &space[0], (size_t) totsize, 1, fp);
      if (size != corrsize) {
      ...
      for(i = 0; i < count; i++) {
        for (j = size-1; j >= 0; j--) {
          if (freord!=0)
            ...
            k = j;
          if (size != corrsize)
            if ((fendian == SEI_FILE_BIGENDIAN && (freord==0)) ||
    ...
    if (serr != null) {
      serr.setLength(0);
// Who cares about the length of the string...?
//      serr.append("Ephemeris file is damaged. ");
//      if (strlen(serr) + strlen(swed.fidat[ifno].fnam) < AS_MAXCH - 1)
        serr.append("Ephemeris file " + swed.fidat[ifno].fnam + " is damaged.");
    }
    fp.close();
    fp = null;
    throw ioe;
...
//#ifdef TRACE0
  FileData() {
    Trace.level++;
    Trace.trace(Trace.level,"FileData()");
    Trace.level--;
  }
//#endif /* TRACE0 */
